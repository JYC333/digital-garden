...
xhr.onreadystatechange = function () {
  if (xhr.readyState == 4 && xhr.status == 200) {
    ...
    arrItems = Array.from(arrItems).filter(
      (d) => d.getElementsByTagName("description")[0].childNodes.length !== 0
    );
...
function searchMatching(arr1, arr2, input) {
  ...
  for (i = 0; i < itemLength; i++) {
    if (arr1[i].search(input) !== -1 || arr2[i].search(input) !== -1) {
      ...
      var step = 50;
      var start = indexContent - step < 0 ? 0 : indexContent - step;
      console.log(
        arr[i],
        input,
        arr[i].slice(start, indexContent),
        arr[i].slice(indexContent, indexContent + l),
  ...
  for (i = 0; i < arrResults.length; i++) {
    var itemDiv = tmpDiv.cloneNode(true);
    itemDiv.innerHTML =
      "<b>" + arrTitles[indexItem[i]] + "</b><br>" + arrResults[i];
    itemDiv.setAttribute(
      "onclick",
      "{changeHref(arrLinks[indexItem[" + i + "]]);closeSearch();}"
    );
    searchResults.appendChild(itemDiv);
  }
}
...
function closeSearch(event) {
  if (
    event.target == document.getElementById("searchOverlay") ||
